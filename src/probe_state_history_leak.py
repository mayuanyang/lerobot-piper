...
def state_geometry(state, ep, args):
    ...
    print(f"\n  Read the LAST column: below ~50% of the gap the state still")
    print(f"  identifies which demonstration this is, and the augmentation buys")
    print(f"  little. The sibling value of 0.02 covers only 21% here.")
    print(f"\n  The offset does NOT invalidate the action label. LIBERO actions")
    print(f"  are DELTAS, and a delta serves the relative geometry between")
    print(f"  gripper and object -- which the images still show correctly,")
    print(f"  because nothing in the scene moved. Only one SENSOR was corrupted,")
    print(f"  and the other still carries the truth, so `a` remains the right")
    print(f"  answer. This is the OPPOSITE of the geometric image augmentation")
    print(f"  moe removed, where translating the frame moved the ground truth")
    print(f"  and left the label behind.")
    print(f"\n  So the knob does not trade label correctness against blurring.")
    print(f"  It sets how hard the policy is pushed off proprioception and onto")
    print(f"  vision -- which is the point when the val split says the expert is")
    print(f"  memorising through the state. The remaining ceiling is soft: at")
    print(f"  very large sigma the absolute position becomes useless, and at")
    print(f"  inference it is accurate, so that is information thrown away.")
    print(f"\n  Note the offset is CONSTANT across the history window, so every")
    print(f"  first difference survives intact -- section 10 measured that")
    print(f"  motion signal as load-bearing. Only the absolute position, which")
    print(f"  is the memorisation key, is blurred.")
