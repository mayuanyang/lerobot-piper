"""
Training script for pretraining WiltechsVLA on the HuggingFaceVLA
...
Key differences from `train_libero_interleaved.py` (single-dataset LIBERO train):
  - Uses WiltechsVLA (Qwen3-VL backbone) instead of SmolVLM2-based interleaved model.
...
def _resize_camera_to_canonical(img: torch.Tensor) -> torch.Tensor:
    ...
    Pad value: 0.0 — Qwen3-VL processor will normalise internally.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pretrain WiltechsVLA on HuggingFaceVLA/community_dataset_v3",
    )
