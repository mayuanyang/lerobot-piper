...
| **2026-06-11** | **WiltechsVLA** | **34k bs96 contrast0.1+vkvdrop0.3** | **10-12** | **64** | **87** | **83** | **73** | **58** | — | **Long 58% (120 episodes, 12 per task).** T1 perfect (12/12), T3 near-perfect (11/12), T2/T5 67%, T9 58%, T7 50%, T4 42%, T0 25%, T6/T8 17%. Per-task: T0=25 (both alphabet soup + tomato sauce → basket), T1=100 ⭐ (both cream cheese + butter → basket), T2=67 (turn on stove + moka pot), T3=92 ⭐ (bowl → drawer + close), T4=42 (mug disambiguation), T5=67 (book → caddy), T6=17 (mug→plate + pudding→right), T7=50 (both alphabet soup + cream cheese → basket), T8=17 (both moka pots → stove), T9=58 (mug → microwave + close). T8 (both moka pots → stove) at 17% — not the zero-collapse seen in interleaved but still weak. T1/T3 are the standout wins: multi-pick same-dest and 3-step articulated tasks work well. T6 (mug→plate + pudding→right) weakest at 17% — dual-object dual-dest precision task. |
...
## WiltechsVLA (Qwen3-VL-4B encoder-decoder) — spatial per-task

Frozen Qwen3-VL-4B-Instruct (bf16, non-FP8) encoder runs once → trainable DiT
decoder (16 layers, `dit_hidden_size=1280`) cross-attends to cached VLM KV.
Training config (both checkpoints): `batch_size 192` (≈96 effective after the
worker pixel pipeline — see run command), `use_8bit_adam`, `preprocess_in_workers`,
**`contrastive_loss_weight 0`** (language-forcing OFF), QFormer gate init 0.1,
8 latent tokens. FPS=10, `n_action_steps=64`.

### libero_spatial (76% @ 20k; was 79% @ 10k)

**Measured 2026-06-10.** 100 episodes, 10 episodes × 10 tasks.

| Task | Description | 10k | **20k** | Δ | Notes |
|------|------|------|------|------|------|
| 0 | between plate and ramekin | 9 | **7** | -2 | regression |
| 1 | next to the ramekin | 10 | **8** | -2 | regression |
| 2 | from table center | — | **9** | — | |
| 3 | on the cookie box | — | **9** | — | |
| 4 | top drawer of wooden cabinet | — | **9** | — | |
| **5** | **on the ramekin** | **0** | **9** | **+9** ⭐⭐⭐ | **Stopping bug FIXED.** Worst failure at 10k (complete collapse, 0/10) → recovered to 9/10. Biggest single-task win. |
| 6 | next to cookie box | 10 | **9** | -1 | |
| 7 | on the stove | 6 | **8** | +2 | stove side improving |
| 8 | next to plate | 8 | **8** | 0 | |
| **9** | **on the wooden cabinet** | **0** | **0** ⚠️⚠️ | **0** | **Persistent structural failure — 0/10 at both 10k and 20k.** See video review below. |

**T9 video review (2026-06-10, 20k):** instead of placing the bowl on the
**wooden cabinet** (the instruction), the model picks up the bowl and puts it on
the **stove** — i.e. it executes **task 7's behavior** (stove) when given task 9's
instruction (cabinet). This is the **stove/cabinet language-confusion** failure
documented across every prior model family (interleaved T9 cabinet bias, wilro):
the model ignores the "wooden cabinet" token and defaults to its dominant stove
prior.

**This directly corroborates the language-conditioning concern**: this run was
trained with `contrastive_loss_weight 0`, removing the only explicit pressure to
attend to language. The model can place a bowl (T0-T8 mostly ≥8), it just won't
condition on "cabinet" vs "stove". Same root cause as the historical goal-suite
9% collapse — language present in the batch (verified: `_encode_language` →
`vlm_seq = cat([vis, lang])` → DiT cross-attn) but not *used*. Cf.
`feedback_data_flow_first`: language IS in the forward batch; the failure is that
nothing forces the model to weight it.

**Recommendation:** re-enable contrastive (e.g. `contrastive_loss_weight 0.1`) and
re-eval T9 specifically — if T9 moves off 0, language-forcing is the lever; if it
stays 0, the cabinet failure is a deeper visual-prior / placement issue independent
of language conditioning.

### libero_spatial (76% @ 28k; contrastive 0.1 + vision_kv_dropout 0.3)

**Measured 2026-06-11.** Resumed the 20k checkpoint with `--contrastive_loss_weight
0.1 --vision_kv_dropout_prob 0.3` plus the latents_wrong contrastive fix. 8k steps
of language-forcing. Training-side: contrastive hinge fell 51%→~33% of margin and
plateaued; x-attn language share ~85-89%.

| Task | Description | 20k | **28k** | Δ | Notes |
|------|------|------|------|------|------|
| 0 | between plate and ramekin | 7 | **8** | +1 | |
| 1 | next to the ramekin | 8 | **10** | +2 | **Perfect** |
| 2 | from table center | 9 | **10** | +1 | **Perfect** |
| 3 | on the cookie box | 9 | **10** | +1 | **Perfect** |
| 4 | top drawer of wooden cabinet | 9 | **10** | +1 | **Perfect** |
| **5** | **on the ramekin** | 9 | **4** | **−5** ⚠️ | **Stopping-bug task regressed.** Was the 10k→20k recovery star (0→9), now 4. |
| 6 | next to cookie box | 9 | **10** | +1 | **Perfect** |
| 7 | on the stove | 8 | **7** | −1 | |
| **8** | **next to plate** | 8 | **2** | **−6** ⚠️⚠️ | **New collapse.** Largest regression; needs video review. |
| **9** | **on the wooden cabinet** | **0** | **5** | **+5** ⭐⭐⭐ | **STRUCTURAL FAILURE BROKEN.** 0/10 at 10k AND 20k → 5/10 after 8k steps of contrastive. Language-forcing confirmed as the lever. |

**Analysis**: total flat (76→76) but the experiment's question is answered: **T9
moved off zero the first time language-forcing was applied**, after sitting at 0
across 10k and 20k without it. Five tasks now perfect (T1-T4, T6) — broad +1 gains
suggest general quality also improved. The cost: T5 (−5) and T8 (−6). Hypothesis
for the collateral: contrastive forces near-synonymous instructions apart
("next to the ramekin" T1 / "next to the cookie box" T6 / "next to the plate" T8
share surface form), and T8 drew the short straw in the re-mapping; T5's stopping
bug is historically regularization-sensitive and may be reacting to vision-KV
dropout. Only 8k steps in — redistribution may still be settling. Next: video
review T5/T8 failure modes (wrong-bowl pick = language confusion vs. stopping vs.
placement precision), train to ~40-50k, re-eval before touching any knobs.

**Confirmed bench-local ↔ parquet mapping (2026-06-11)** + lexical-collision read
of the 28k results. All instructions share the suffix "…and place it on the plate";
the qualifier phrase is the only discriminative signal:

| Bench | Parquet | Qualifier phrase | 28k | Lexical situation |
|------|------|------|------|------|
| T0 | 34 | between the plate and the ramekin | 8 | "between" semi-unique |
| T1 | 37 | next to the ramekin | 10 | minimal pair with T5 — WINNER |
| T2 | 38 | from table center | 10 | "table center" unique |
| T3 | 35 | on the cookie box | 10 | "cookie box" shared w/ T6 only |
| T4 | 31 | in the top drawer of the wooden cabinet | 10 | "top drawer" unique |
| T5 | 32 | on the ramekin | **4** ⚠️ | minimal pair with T1 (differs only "on"/"next to") — LOSER |
| T6 | 30 | next to the cookie box | 10 | |
| T7 | 33 | on the stove | 7 | "stove" unique |
| T8 | 36 | next to the plate | **2** ⚠️ | qualifier noun = universal destination noun ("plate") — ZERO unique content words, lowest lang SNR — LOSER |
| T9 | 39 | on the wooden cabinet | **5** ⭐ | "wooden cabinet" distinctive — contrastive had a clean feature |

**Pattern**: every task with a unique content word scored 7-10; both losers are
tasks distinguishable only by prepositions ("on" vs "next to") or by a noun that
collides with the shared suffix. Contrastive separation resolved minimal pairs as
winner/loser (T1=10/T5=4; T0=8/T8=2) instead of genuinely discriminating —
evidence that the frozen VLM's language KVs (raw-string tokenization, no chat
template, no vision_start/end markers) under-represent prepositional distinctions.
If T5/T8 don't recover by 40-50k, the chat-template input format is the next
experiment; a softer alternative is excluding/down-weighting same-scene minimal
pairs in the contrastive permutation.
**RESOLVED at 34k**: T5/T8 recovered on their own (4→8, 2→8) — see next section.

### libero_spatial (87% @ 34k; contrastive 0.1 + vision_kv_dropout 0.3) ⭐ ties repo-best

**Measured 2026-06-11** (eval 77.3 s/ep, checkpoint `ISdept/wiltechs-vla-34k`,
legacy input format — NO chat template; the chat-template resume starts at 34k
and trains 36k+, so this is its control anchor). 6k more steps of the same
contrastive 0.1 + vkv-dropout 0.3 recipe. Training-side at 32-35k: contrastive
hinge 12-19% of margin (down from the 21-27k plateau at ~33%).

| Task | Description | 28k | **34k** | Δ | Notes |
|------|------|------|------|------|------|
| 0 | between plate and ramekin | 8 | **9** | +1 | |
| 1 | next to the ramekin | 10 | **9** | −1 | |
| 2 | from table center | 10 | **10** | = | **Perfect** |
| 3 | on the cookie box | 10 | **9** | −1 | |
| 4 | top drawer of wooden cabinet | 10 | **8** | −2 | |
| **5** | **on the ramekin** | 4 | **8** | **+4** ⭐ | **Self-recovered** — 28k winner/loser collapse was transient |
| 6 | next to cookie box | 10 | **10** | = | **Perfect** |
| 7 | on the stove | 7 | **7** | = | Now the suite floor |
| **8** | **next to plate** | 2 | **8** | **+6** ⭐⭐ | **Self-recovered** — biggest 28k casualty fully back |
| **9** | **on the wooden cabinet** | 5 | **9** | **+4** ⭐⭐⭐ | **0 (20k) → 5 (28k) → 9 (34k).** Historical structural failure near-solved |

**Analysis**: 76 → 87 (+11), tying the repo-best spatial (interleaved 74k = 87)
at 34k steps vs 74k. The three findings:

1. **Contrastive language-forcing fully validated.** T9's trajectory 0→5→9 over
   14k steps of language-forcing is monotone and large; nothing else changed.
2. **The 28k winner/loser re-mapping was transient, not structural.** T5 and T8
   both recovered to 8 without any knob change — the contrastive pressure first
   tears near-synonymous instructions apart (winner/loser), then re-discriminates
   them given more steps. The "minimal pairs need the chat template" hypothesis
   is NOT refuted (the template may still raise the ceiling/speed) but it is no
   longer needed to explain T5/T8.
3. **No catastrophic task left** — floor is T7 (stove) at 7. The suite is now a
   uniform-polish problem, not a structural-failure problem. n=10 caveat: 87±~7;
   confirm with n=50 before any external SOTA-comparison claim.

**Next**: chat-template resume evals (40k+) compare against THIS 87, not 76.
Open question for the template run: does it push the minimal-pair tasks (T5/T8)
and T7 above 8-9, and does the libero_object weakness improve.

### libero_object (83% @ 34k; n=12/task, 120 episodes)

**Measured 2026-06-11** (eval 46.7 s/ep, same `ISdept/wiltechs-vla-34k` legacy
checkpoint as the spatial 87%). The earlier "very bad" quick-run impression did
NOT reproduce at full eval: 83.3% matches wilro's best object (wilro-ed-s2 69k =
83) but trails interleaved 74k (93) by 10. Note n=12 per task (not 10), so
scores below are x/12.

| Bench task | 34k (of 12) | % | Notes |
|------|------|------|------|
| T0 | 9 | 75 | |
| T1 | 11 | 92 | |
| T2 | **7** | **58** ⚠️ | suite floor — episodes 2,4,5,6,7 failed (a mid-eval cluster) |
| T3 | 12 | 100 | **Perfect** |
| T4 | 12 | 100 | **Perfect** |
| T5 | **8** | **67** ⚠️ | |
| T6 | 10 | 83 | |
...
**Analysis**: 3 perfect tasks, no zero-collapse anywhere — this is a polish
problem, not a structural one. Failures concentrate in T2 (5/12 failed) and T5
(4/12). Bench-id ↔ object-name mapping not yet confirmed for the object suite
(spatial taught us bench order ≠ parquet order) — identify T2/T5's objects from
the failure videos before theorizing. The key unanswered question is failure
TYPE: wrong-object pick (semantic binding → VLM/template side) vs right-object
fumble (grasp/placement precision → robot-CNN side). Videos for T2 episodes
2,4-7 and T5 episodes 4,7,8,11 answer this directly.

### libero_goal (73% @ 34k; n=12/task, 120 episodes)

**Measured 2026-06-11** (eval 56.2 s/ep, same `ISdept/wiltechs-vla-34k` legacy
checkpoint). 73.3% matches interleaved's goal ceiling (72-74) but trails wilro
badly (wilro-ed-l16 99k = 91, wilro-ed-s2 69k = 85). Goal is WiltechsVLA's
weakest suite, with two structural-grade failures:
...
| T1 | 11 | 92 | |
| T2 | 11 | 92 | |
| **T3** | **4** | **33** ⚠️⚠️ | failures ep 0,3,5,6,7,8,9,10 |
| T4 | 11 | 92 | |
| T5 | 10 | 83 | |
| T6 | 9 | 75 | |
| T7 | 12 | 100 | **Perfect** |
| **T8** | **7** | **58** ⚠️ | failures ep 2,4,6,8,10 |
| **T9** | **2** | **17** ⚠️⚠️⚠️ | failures ep 0-9 (ALL rendered videos are failures), successes only ep 10-11 |

**Analysis**: 7 of 10 tasks at ≥75% with T7 perfect — the suite's 73 average is
entirely T3+T8+T9 (lose 21 of the 32 total failures). The goal suite contains
the stove/cabinet and rack/cabinet minimal-pair instructions ("put the bowl on
the stove" / "on top of the cabinet"; "wine bottle on the rack" / "on top of
the cabinet") — the same confusion family as spatial T9 and the historical
interleaved goal weakness (interleaved never exceeded 74 here; its T9
wine-rack sat at 4-5). Bench-id ↔ task-name mapping NOT yet confirmed for goal;
identify T3/T8/T9's instructions from the videos before theorizing. T9's
videos are maximally convenient: all 10 rendered episodes are failures.
Classify: wrong-target placement (stove/cabinet-style language confusion →
contrastive/template side) vs multi-step breakdown (drawer-open + place
sequences) vs motor fumble.

**Standing (34k legacy)**: spatial 87 (ties repo-best) + object 83 (ties
wilro-best) + goal 73 (matches interleaved, −18 vs wilro-l16's 91). avg3 = 81
vs wilro-ed-l16 99k 84.3, interleaved 74k 84, wilro-ed-s2 69k 82.7. Goal is THE
